...
public class SystemInfo
{
    ...
    public SystemInfo()
    {
        ...
        _logger.Information(
            "Available Vulkan layers: {VulkanLayers}",
            AvailableLayers.Select(SilkExtensions.GetLayerName)
        );
        _logger.Information(
            "Available Vulkan extensions: {VulkanExtensions}",
            AvailableExtensions.Select(SilkExtensions.GetExtensionName)
        );
        _logger.Information(
            "Vulkan validation layers available: {ValidationAvailable}",
            IsValidationLayersEnabled
        );
        _logger.Information(
            "Vulkan debug utils available: {UtilsAvailable}",
            IsDebugUtilsAvailable
        );
    }
    ...
    public bool IsLayerAvailable(string name) =>
        AvailableLayers.Any(properties => properties.GetLayerName() == name);

    public bool IsExtensionAvailable(string name) => _vk.IsInstanceExtensionPresent(name);

    public bool IsLayerExtensionAvailable(string layerName, string extensionName) =>
        _vk.IsInstanceExtensionPresent(extensionName, layerName);

    private IEnumerable<LayerProperties> FetchLayers()
    {
        // ReSharper disable once ConvertToConstant.Local
        uint layerCount = 0;
        var res = _vk.EnumerateInstanceLayerProperties(
            ref layerCount,
            ref Unsafe.NullRef<LayerProperties>()
        );
        if (res != Result.Success)
            return Enumerable.Empty<LayerProperties>();

        var availableLayers = new LayerProperties[layerCount];
        res = _vk.EnumerateInstanceLayerProperties(
            ref layerCount,
            ref availableLayers.DangerousGetReference()
        );
    ...
    private IEnumerable<ExtensionProperties> FetchExtensions(string? layerName = null)
    {
        ...
        var res = _vk.EnumerateInstanceExtensionProperties(
            layerName,
            ref extensionCount,
        ...
        res = _vk.EnumerateInstanceExtensionProperties(
            layerName,
            ref extensionCount,
        ...
        return res != Result.Success
            ? Enumerable.Empty<ExtensionProperties>()
            : availableExtensions;
    }
}
